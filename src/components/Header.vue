  ...
  <div class="header">
    ...
    <div class="right">
      <div class="btn" @click="openSetting">
        <span class="icon iconfont icon-shezhitianchong"></span> Settings
      </div>
      <div class="btn" @click="openTemplate">
        <span class="icon iconfont icon-moban"></span> Templates
      </div>
      <div class="dropdownBtn" @click.stop>
        <div class="btn" @click="toggleToolsList()">
          <span class="icon iconfont icon-gongju"></span>  &nbsp;Tools
        </div>
        <ul class="toolList" :class="{ show: showToolsList }">
          <li class="toolItem" @click="exportZipFile">export zip</li>
      ...
      <div class="dropdownBtn" @click.stop>
        ...
        <ul class="toolList" :class="{ show: showMoreList }">
          <li class="toolItem" @click="createNew">Create new project</li>
          <li class="toolItem" @click="showMyGists">Show my gists</li>
          <li class="toolItem" @click="githubToken ? logout() : login()">
            {{ githubToken ? 'Log Out' : 'Log In' }}
          </li>
        </ul>
      </div>
    </div>
    ...
    >
      <div class="settingBox">
        <el-tabs tab-position="left" v-model="settingType">
          <el-tab-pane label="Layout" name="layout"></el-tab-pane>
          <el-tab-pane label="Theme" name="theme"></el-tab-pane>
          <el-tab-pane label="Settings" name="setting"></el-tab-pane>
        </el-tabs>
    ...
    >
      ...
      <p class="tip">
        If you do not have a github token，create a new one <a
          href="https://github.com/settings/tokens/new?scopes=repo"
          target="_blank"
          >here</a
        >.<br> Remember to select【scopes】under the【gist】option。
      </p>
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="cancelGithubTokenValueInput">Cancel</el-button>
...
const useLogin = ({ store, router }) => {
  ...
  const confirmGithubTokenValueInput = () => {
    ...
    if (!trimValue) {
      ElMessage.warning('Please input your token')
      return
    }
...
const useDrawer = ({ router, route, githubToken, login }) => {
  ...
  const onDrawerOpen = async () => {
    try {
      ...
      gistloading.value = false
      ElMessage.error('Failed to retrieve.')
    }
  }
  ...
  const deleteGist = async id => {
    try {
      ...
      gistloading.value = false
      ElMessage.success(
        'successfully deleted. Note: deletion is not a synchronized process. Please do not delete again within five minutes.'
      )
